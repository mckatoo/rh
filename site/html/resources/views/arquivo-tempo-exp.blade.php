            ...
            <div class="col-lg-12">
                ...
                <div class="col-lg-12">
                    <form name=form role="form" method="post" enctype="multipart/form-data">
                        {!! csrf_field() !!}
                        @if(isset($id_tempo_exp))
                            <input type="hidden" name="id_tempo_exp" value={{$id_tempo_exp}}>
                        @endif
                        @if(isset($id_prof))
                        ...
                        <div class="form-group col-lg-12">
                            <fieldset>
                                <legend>TEMPO EXPERIÊNCIA PROFISSIONAL COMPROVADA (em anos - fora o magistério).</legend>
                                <small>
                                    USE O BOTÃO ABAIXO PARA INSERIR OS DOCUMENTOS EM PDF QUE COMPROVAM O VALOR INFORMADO NA TELA ANTERIOR REFERENTE A TEMPO DE EXPERIÊNCIA PROFISSIONAL EXCETO MAGISTÉRIO.
                                </small>
                                <br>
                                <br>
                                ...
                                @if((isset($tempo_exp->arquivos))and(count($tempo_exp->arquivos)>0))
                                    <br>
                                    <br>
                                    <table class="table table-bordered table-striped table-hover">
                                        ...
                                        <tbody>
                                            @foreach($tempo_exp->arquivos as $arquivo)
                                            <tr>
                                                <td class="middle">{!! $arquivo->arquivo !!}</td>
                                                <td class="middle">{!! $arquivo->created_at->format('d/m/Y H:i') !!}</td>
                                                <td class="middle tdbtn">
                                                    <input type="hidden" name="id_arq" value="{{$arquivo->id}}">
                                                    <a href="{!! route('tempo_exp.download', $arquivo->id) !!}" class="btn btn-xs btn-success">download</a>
                                                </td>
                                                <td class="middle tdbtn">
                            ...
                            <a class="btn btn-primary pull-left" href="{{ route('tempo_mag.index') }}">VOLTAR</a>
                            {{-- @if ($tempo_exp->arquivos->count() === 0) --}}
                                {{-- <a href="{{ route('telefone_prof.index') }}" class="btn btn-primary pull-right disabled">PRÓXIMO</a> --}}
                            {{-- @else --}}
                                <a href="{{ route('telefone_prof.index') }}" class="btn btn-primary pull-right">PRÓXIMO</a>
                            {{-- @endif --}}
                        </div>
                    </form>
                </div>
