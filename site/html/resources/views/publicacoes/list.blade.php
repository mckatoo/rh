<div class="form-group col-lg-12">
    <fieldset>
        <legend>USE O BOTÃO ABAIXO PARA ADICIONAR ARQUIVO EM PDF CONTENDO A PUBLICAÇÃO.</legend>
        <span class="btn btn-success fileinput-button">
            <i class="glyphicon glyphicon-plus"></i>
            <span>Selecionar arquivos...</span>
            {!! Form::file('documento', ['id'=>'fileupload','accept'=>'application/pdf']) !!}
            {!! Form::hidden('idPublicacao', null, ['id'=>'idPublicacao']) !!}
        </span>
        <br>
        <br>
...
@if((isset($arq))and(count($arq)>0))
    ...
    <table class="table table-bordered table-striped table-hover">
        ...
        <tbody>
            ...
            <tr>
                <td class="middle"><a href="{{ route('publicacoes.download',[$arquivo->id]) }}">{{ $arquivo->arquivo }}</a></td>
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-footer">
                                    ...
                                    post('{{route('publicacoes.excluir_arq')}}',
                                    {
                                        _token: '{{csrf_token()}}',
                                        idArq: '{{$arquivo->id}}',
                                        idPublicacao: '{{$arquivo->publicacao_id}}'
                                    },
                                    '{{ route('publicacoes.list_arq',$idPublicacao) }}');
...
;(function($)
{
    ...
    $(document).ready(function()
    {
        ...
        $fileupload.fileupload({
            url: "{{ route('publicacoes.upload') }}",
            formData: {_token: "{{ csrf_token() }}", idPublicacao: "{{ $idPublicacao }}"},
            ...
            complete: function (e, data) {
                $("#bind").load("{{ route('publicacoes.list_arq',$idPublicacao) }}");
            }
        });
    });
})(window.jQuery);
